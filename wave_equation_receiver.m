...
while (t<T)
    ...
    for i=2:nx-1
        for j=2:nz-1
            ...
            tau2xp1(i,j)=(1-dt*di/2)*tau2x(i,j)/(1+dt*di/2)+Mavg*dt*(u(i+1,j)-u(i,j))/((1+dt*di/2)*dx);
            tau2zp1(i,j)=(1-dt*dj/2)*tau2z(i,j)/(1+dt*dj/2)+Mavg*dt*(v(i,j+1)-v(i,j))/((1+dt*dj/2)*dz);
        end
    end
...
while (t>0)
    ...
    for ii=1:nx
        for jj=1:nz
            nu = 0.5 * L(ii,jj)/(L(ii,jj) + M(ii,jj));
            E = 2.0*M(ii,jj)*(1.0+ nu);
            % Compute total field from split components
            epsilon_xx = ( tau1(ii,jj) - nu*tau3(ii,jj) )/E;
            epsilon_zz = ( tau3(ii,jj) - nu*tau1(ii,jj) )/E;
            epsilon_xz = 2.0*(1.0+ nu)*tau2(ii,jj) / E;

            energy(ii, jj) = 0.50 * ( 0.5*(ro+ro)* vx(ii,jj)^2 ... 
            + 0.5*(ro+ro)*vz(ii,jj)^2) + ...
            0.50 * ( epsilon_xx * tau1(ii,jj) + epsilon_zz * tau3(ii,jj) + epsilon_xz * tau2(ii,jj));
    
            maxEnergy(ii, jj) = max(maxEnergy(ii, jj), tau3(ii, jj));
        end
    end
