...
! DD
integer, parameter :: iprocs = 1, jprocs = 1

! Wave equation parameters
real(8), parameter :: dx = 20.0, dz = 20.0
real(8), parameter :: r = 2600.0, time_global = 0.2
real(8), parameter :: CFL = 0.5
real(8) :: dt

! Wave source
! Source in the global coordinate
integer, parameter :: isrc = 50, jsrc = 52
integer :: local_isrc, local_jsrc
real(8), parameter :: half_dur = time_global / 2.0
real(8) :: time = 0.0
integer :: i, j
...
! Domain
integer, parameter :: nxlocal = 200, nzlocal = 200
integer, parameter :: nx=nxlocal, nz=nzlocal
...
real(8), dimension(:,:), allocatable :: BU, LAM, MU, GAMMA
...
character*60 :: input_Vp_filename="Model_clatter_Vp_S_20_I_4 2.mod"
...
do while (tt < t)
    ...
    call mpi_send(send2, 3*nx, MPI_REAL8, jnext, 2000, MPI_COMM_WORLD, error)
    call mpi_send(send1, 3*nx, MPI_REAL8, jprev, 1000, MPI_COMM_WORLD, error)
    
    call mpi_recv(recv2, 3*nx, MPI_REAL8, jprev, 2000, MPI_COMM_WORLD, status, error)
    call mpi_recv(recv1, 3*nx, MPI_REAL8, jnext, 1000, MPI_COMM_WORLD, status, error)
    ...
    ! source 
    if ((isrc .gt. ranki*nxlocal .or.  isrc == ranki*nxlocal ).and. isrc .lt. (ranki+1)*nxlocal .and. &
        (jsrc .gt. rankj*nzlocal .or.  jsrc == rankj*nzlocal) .and. jsrc .lt. (rankj+1)*nzlocal) then 
        ...
        !write(*,*) tau3(local_isrc, local_jsrc)
    end if 
